...
import { addCell } from '../../common/cellService';
...
export function AddRowForm(props: AddRowFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    addCell(`${apiUrl}/executeSqlQuery`, columnDefs, `${tableName}`, inputFieldStates)
      .then(() => {
        window.location.reload();
      })
      .catch((error) => {
        setOpenStatusModal(true);
        setStatusModalMessage(`Error adding row: ${String(error.message)}`);
      });
  };
